...
/*no need to create codecommit repo, resource "aws_codecommit_repository" "source" {
  repository_name = "${var.project_name}-${var.name}"
}*/
...
resource "aws_codebuild_project" "_" {
  ...
  environment {
    ...
    environment_variable {
      ...
      value = var.cloudfront_distribution.id
    }

    environment_variable {
      name  = "DEPLOY_SCRIPT"
      value = var.deploy_script_env
    }
  }
}
